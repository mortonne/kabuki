...
class TestStepMethods(unittest.TestCase):
    ...
    def test_uniform_normalstd_with_multiple_condition_numerical_solution(self):
        """test uniform_normalstd with Metropolis to evaluate the numerical solution of
        the mean and std"""
        self.uniform_normalstd_multiple_conds_with_shared_sigma(sigma_0=0.5, mu_0=(0, 2, 10), n_subjs=8, seed=1,
                                                                use_metropolis=True)
        self.uniform_normalstd_multiple_conds_with_shared_sigma(sigma_0=3.5, mu_0=(-100, 3), n_subjs=4, seed=2,
                                                                use_metropolis=True)
        self.uniform_normalstd_multiple_conds_with_shared_sigma(sigma_0=2.5, mu_0=(1, 2), n_subjs=5, seed=3,
                                                                use_metropolis=True)
        self.uniform_normalstd_multiple_conds_with_shared_sigma(sigma_0=0.5, mu_0=(-4, -3, 2, 1, 0), n_subjs=7, seed=4,
                                                                use_metropolis=True)
    ...
    def test_UniformNormalstd_step_method_with_multiple_condition(self):
        ...
        self.uniform_normalstd_multiple_conds_with_shared_sigma(sigma_0=2.5, mu_0=(1, 2), n_subjs=5, seed=3,
                                                                use_metropolis=False)
        self.uniform_normalstd_multiple_conds_with_shared_sigma(sigma_0=0.5, mu_0=[-4], n_subjs=7, seed=4,
                                                                use_metropolis=False)
        ...
        while i_try < max_tries:
            ...
            try:
                for i in range(len(df) / 3):
                    np.testing.assert_allclose(df[(3 * i + 0):(3 * i + 1)], df[(3 * i + 1):(3 * i + 2)], atol=0.1,
                                               rtol=0.01)
                    np.testing.assert_allclose(df[(3 * i + 1):(3 * i + 2)], df[(3 * i + 2):(3 * i + 3)], atol=0.1,
                                               rtol=0.01)
                    np.testing.assert_allclose(df[(3 * i + 2):(3 * i + 3)], df[(3 * i + 0):(3 * i + 1)], atol=0.1,
                                               rtol=0.01)
        ...
        while i_try < max_tries:
            ...
            try:
                for i in range(len(df) / 2):
                    np.testing.assert_allclose(df[(2 * i + 0):(2 * i + 1)], df[(2 * i + 1):(2 * i + 2)], atol=0.1,
                                               rtol=0.01)
                break
            # if not add more runs
            except AssertionError:
