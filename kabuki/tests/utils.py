...
class HNodeSimpleVar(kabuki.Hierarchical):
    def create_knodes(self):
        if self.is_group_model:
            ...
            mu_tau = Knode(pm.Deterministic, 'mu_tau', doc='mu_tau', eval=lambda x: x ** -2, x=mu_std, plot=False,
                           trace=False)
...
def create_test_models():
    n_subj = 5
    data, params = kabuki.generate.gen_rand_data(gen_func_df,
                                                 {'A': {'loc': 0, 'scale': 1}, 'B': {'loc': 0, 'scale': 1}},
                                                 subjs=n_subj, size=20)
